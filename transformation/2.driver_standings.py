...
driver_standing_spec = Window.partitionBy("race_year").orderBy(desc("total_points"),desc("wins"))

final_df = driver_standing_df.withColumn("rank", rank().over(driver_standing_spec))
...
merge_condition = "tgt.driver_name = src.driver_name AND tgt.race_year = src.race_year" # first one can be ideal for primary key
